  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const payload = {
        ...
        role: formData.role,
        // Include adminKey in payload (backend ignores it for non-admin roles)
        ...(formData.role === "ADMIN" && { adminKey }),
      };

      await apiService.register(payload);

      // For non-admin roles: auto-login after registration
      if (formData.role !== "ADMIN") {
        try {
          ...
          navigate(`/${loginData.user.role.toLowerCase()}dashboard`);
          return;
        } catch {
          // If auto-login fails, fall through to manual login redirect
        }
      }

      // Admin registrations redirect to login manually
      setTimeout(() => {
        navigate("/login");
      }, 1000);
    ...
    <div className="auth-container">
      ...
      <div className="auth-card">
        ...
        <form onSubmit={handleSubmit} className="auth-form">
          ...
          {/* Admin Private Key — only shown when ADMIN role is selected */}
          {formData.role === "ADMIN" && (
            <div className="form-group">
              ...
              <div className="password-hint" style={{ color: '#e67e22' }}>
                ⚠️ Admin accounts require a private key provided by your organization.
              </div>
            </div>
          )}
