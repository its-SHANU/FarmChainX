...
function FarmerDashboard({ products, onDeleteProduct, onLogout, onRefreshProducts }) {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      console.log('🔄 Refreshing products from backend...');

      // Use the callback from App.jsx if available, otherwise fetch directly
      if (onRefreshProducts) {
        ...
        if (response.ok) {
          ...
          showNotification('Failed to refresh products from server', 'error');
        }
      }
    } catch (error) {
      console.error('❌ Refresh error:', error);
      showNotification('Error refreshing products: ' + error.message, 'error');
    } finally {
      setIsLoading(false);
    }
  };
